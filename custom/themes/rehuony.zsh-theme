...
# Strict first-token hostname: dot suffix, then hyphen, then space.
hostname="${${${HOST%%.*}%%-*}%% *}"

# Concatenate prompts
if [[ $EUID -eq 0 ]]; then
  PROMPT="${hotpink}%n@${hostname}%{$reset_color%}"
else
  PROMPT="${purple}%n@${hostname}%{$reset_color%}"
fi

PROMPT="${PROMPT} in ${turquoise}%2~%{$reset_color%}\$(virtualenv_prompt_info)\$(ruby_prompt_info)\$vcs_info_msg_0_${orange} λ%{$reset_color%} "
